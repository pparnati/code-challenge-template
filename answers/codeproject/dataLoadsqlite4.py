    ...
    if os.path.isfile(os.path.join("../wx_data", wxFile)):
        ...
        for line in lines:
            ...
            try:
                coxn = conn.cursor()
                year=row[0]
                ss= "insert into codeapp_WeatherData(weatherdate,weatherstation,maxtempofday,mintempofday,precipofday,theyear) values (" + \
               year + ",'" + wxFile[0:11] + "'," + row[1] + "," + row[2] + "," + row[3] +"," + year[0:4]+ ")"
                print("Passed "+ss)
                coxn.execute(ss)
                print("Passed 3")
                #for i in coxn:
